...
const HomeContent = () => {
  ...
  const fetchRandomCity = async () => {
    try {
      const response = await fetch("https://test-demo-r76u.onrender.com/api/destinations/random");
  ...
  const submitAnswer = () => {
    try {
      ...
      setScore((prev) => prev + correctSelected.length - incorrectSelected.length);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 to-purple-500 text-white p-6">
      ...
      <h1 className="text-4xl font-extrabold mb-4">🌍 Globetrotter Challenge</h1>
      {city && <h2 className="text-2xl mt-4 font-semibold">Select the correct clues for {city}! 🏙️</h2>}
        ...
        {clues.map((clue, index) => (
          <motion.div
            ...
            className={`p-4 border-2 rounded-xl cursor-pointer text-lg font-semibold transition-all duration-300 shadow-lg 
              ${selectedClues.includes(clue) ? "bg-green-500 text-white" : "bg-white text-black"}`}
            whileTap={{ scale: 0.95 }}
            onClick={() =>
      ...
      {result && (
        <div className="mt-6 p-6 text-center text-lg font-semibold bg-white text-black rounded-lg shadow-lg w-full max-w-md">
          {result === "success" && (
            <>
              🎉 <span className="text-green-600">Fantastic! You got them all right!</span>
            </>
          )}
          {result === "mixed" && (
            <>
              😊 <span className="text-yellow-600">Good effort! Some were correct, some weren&rsquo;t. Keep going!</span>
            </>
          )}
          {result === "failure" && (
            <>
              😢 <span className="text-red-600">Oops! All incorrect. Try again! You&rsquo;ll get it next time.</span>
            </>
          )}
          <p className="mt-2">✅ Correct: {correct} | ❌ Incorrect: {incorrect}</p>
